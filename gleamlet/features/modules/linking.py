...
def add_iod(
    ...
    ext_data_dir: str | Path | None = None,
) -> pd.DataFrame:
    """
    ...
    *Notes*: The structure of IoD looks like this:
    ...
    external_data_dir = Path(ext_data_dir or DATA_PATHS["EXT_DATA_DIR"])

    if df_onspd is None:
        df_onspd_bd = pd.read_csv(external_data_dir / ExtRefs.ONSPD_BD_FILE)
        df_onspd_leeds = pd.read_csv(external_data_dir / ExtRefs.ONSPD_LS_FILE)
    ...
    if df_iod is None:
        ...
        df_iod = pd.read_csv(external_data_dir / ExtRefs.iod_file(iod_version))
