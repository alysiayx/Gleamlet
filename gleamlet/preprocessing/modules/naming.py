                            ...
                            if len(duplicated_base_names) > 0: # Found duplicated column names
                                ...
                                for base_name in duplicated_base_names:
                                    ...
                                    if len(cols_to_keep) > 1:
                                        identity_label = (
                                            ""
                                            if df.iloc[:, cols_to_keep].nunique(axis=1).max() == 1
                                            else "NOT"
                                        )
                                        logger.warning(
                                            f"Multiple non-empty columns with base name '{base_name}'. "
                                            f"They are {identity_label} identical. Keeping the first one."
                                        )
