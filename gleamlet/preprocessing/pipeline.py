...
# Potential BUG: The FileMetadata.COHORT_YG_AY and FileMetadata.COHORT_Y11_AY fields in file_metadata should follow this format: 'a 4-digit year - 4-digit year' (e.g., 2018 - 2019). Note that there should be spaces on both sides of the hyphen (-). Failure to adhere to this formatting requirement could potentially cause a bug.
# Potential BUG: the process of creating / updateing col_metadata may has bugs (e.g., cannot update correctly)
...
class DataPreprocessor:
    def __init__(
        ...
        default_paths = {
            ...
            "col_metadata_path": self.settings.get_path("col_meta_path"),
            "src_data_dir": self.settings.get_path("raw_dir"),
            "src_meta_dir": self.settings.get_path("raw_meta_dir"),
            "ext_data_dir": self.settings.get_path("external_dir"),
            "src_colstd_dir": self.settings.get_path("interim_dir"),
            "proc_data_dir": self.settings.get_path("processed_dir"),
        }
        paths = {
        ...
        for key, user_path in paths.items():
            default_value = (
                DATA_MANIFEST_PATH
                if key == "data_manifest_path"
                else default_paths[key]
            )
            setattr(self, key, set_default_path(user_path, default_value))
      
        subfolders = {
            "clean_data_dir": (
                self.settings.get_path("cleaned_dir")
                if clean_dir == "1_cleaned"
                else self.proc_data_dir / clean_dir
            ),
            "merge_data_dir": (
                self.settings.get_path("merged_dir")
                if merge_dir == "2_merged"
                else self.proc_data_dir / merge_dir
            ),

            "profile_dir": self.settings.get_path("profile_dir"),
        }       
    ...
    def extract_file_metadata(
        ...
        make_output_dir(output_path, logger=logger)
    ...
    def standardise_fnames_colnames(
        ...
        make_output_dir(output_path, logger=logger)
        make_output_dir(col_metadata_path, logger=logger)
    ...
    def create_data_profile(
        ...
        make_output_dir(self.profile_dir, logger=logger)
    ...
    def merge_data(
        ...
        data_schema: Dict[str, pd.DataFrame] = None,
        output_path: Union[str, Path] = None,
        group_by: Literal["cohort_yg_ay", "cohort_y11_ay", False] = "cohort_yg_ay",
        name_by_year_range: bool = False,
        ...
        name_by_year_range : bool, default False
            Only applies when ``group_by=False``. When False, use the current
            ``prepared`` dataset filename from the merged config. When True, name
            the file from the available cohort span (for example,
            ``2018-2026.parquet``).
        ...
        if name_by_year_range and group_by:
            warnings.warn(
                "name_by_year_range is only valid when group_by=False; "
                "ignoring the name_by_year_range setting."
            )
            name_by_year_range = False

        output_filename = None
        if group_by is False:
            if name_by_year_range:
                spans = (
                    file_metadata[FileMetadata.COHORT_Y11_AY]
                    .astype("string")
                    .str.extract(r"(\d{4})\s*-\s*(\d{4})")
                    .dropna()
                    .astype(int)
                )
                if spans.empty:
                    raise ValueError("No YYYY-YYYY cohort spans were found")
                output_filename = f"{spans[0].min()}-{spans[1].max()}.parquet"
            else:
                output_filename = self.settings.get_path("prepared").name
