...
def merge_data(
    ...
    group_by: Literal["cohort_yg_ay", "cohort_y11_ay", False] = False,
    ...
    if group_by is False and output_filename is not None:
        output_exists = (output_path / output_filename).exists()
    else:
        output_exists = any(output_path.glob("*.parquet"))
    
    if all(not v for v in diff.values()) and output_exists and not overwrite:
        logger.info(f"No changes detected and output files exist at {output_path / output_filename}. Skipping merge process.")
